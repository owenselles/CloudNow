        ...
        Group {
            ...
            case .standard:
                StandardStatsPanel(
                    ...
                    videoDiagnostics: streamController.videoDiagnostics,
                    microphoneEnabled: microphoneEnabled,
                    serverLocation: serverLocation,
...
private struct StandardStatsPanel: View {
    ...
    let videoDiagnostics: VideoPipelineSnapshot
    let microphoneEnabled: Bool
    let serverLocation: String
...
private struct DebugStatsColumn: View {
    ...
    var body: some View {
        StatsSection(title: L10n.text("debug")) {
            StatsRow(label: "Presentation", value: videoDiagnostics.presentationMode)
            StatsRow(
                label: "Callback / enqueue / display",
                value: String(
                    format: "%.0f / %.0f / %.0f fps",
                    videoDiagnostics.callbackFramesPerSecond,
                    videoDiagnostics.enqueuedFramesPerSecond,
                    videoDiagnostics.presentedFramesPerSecond
                )
            )
            StatsRow(
                label: "App drops / superseded",
                value: "\(videoDiagnostics.droppedFrames) / \(videoDiagnostics.supersededFrames)"
            )
            StatsRow(
                label: "Backpressure / pending",
                value: "\(videoDiagnostics.backpressureEvents) / \(videoDiagnostics.pendingFrames)"
            )
            StatsRow(
                label: "AV drops / delay",
                value: String(
                    format: "%d / %.1f ms",
                    videoDiagnostics.avDroppedFrames,
                    videoDiagnostics.avAccumulatedFrameDelayMs
                )
            )
            StatsRow(
                label: "Optimized compositing",
                value: "\(videoDiagnostics.avOptimizedFrames) / " +
                    "\(max(0, videoDiagnostics.avTotalFrames - videoDiagnostics.avDroppedFrames))"
            )
            StatsRow(
                label: "NACK/PLI/FIR",
